...
chessBoard::chessBoard() {
    boardRepresentation = new Board("3R4/1pk3pp/pBp5/8/4P3/1Q5P/PP2qrP1/1K6 b - - 7 31");
    ...
    this->yStart = (Game::h + Game::getPos(0.48, 0)) / 2 - squareLength;
    {
        ...
        _destRect = {Game::getPos(0.22, 0), (Game::h - Game::getPos(0.48, 0)) / 2, Game::getPos(0.48, 0), Game::getPos(0.48, 0)};
...
void chessBoard::Render() {
    ...
    // Numering the Board
    {
        for (int row = 0; row < 8; ++row) {
            if (isReversed) {
                numRow[7 - row]->render(xStart + 3, yStart - row * squareLength + 3);
            } else {
                numRow[row]->render(xStart + 3, yStart - row * squareLength + 3);
            }
        }
        for (int col = 0; col < 8; ++col) {
            if (isReversed) {
    ...
    // Put pieces of Current State
    {
        for (int row = 0; row < 8; ++row) {
            for (int col = 0; col < 8; ++col) {
                pair<Piece, Color> state = boardRepresentation->getSquareState(col, row);
                SDL_Rect src = {0, 0, 150, 150}, dest;
                if (this->isReversed) {
                    dest = {xStart + (7 - col) * squareLength, yStart - (7 - row) * squareLength, squareLength, squareLength};
                } else {
                    dest = {xStart + col * squareLength, yStart - row * squareLength, squareLength, squareLength};
                }
                switch (state.second) {
